...
async function startServer() {
  ...
  // API Route for analyzing HR Data
  app.post("/api/analyze-hr", async (req, res) => {
    try {
            ...
            config: {
              ...
              responseSchema: {
                ...
                items: {
                  type: Type.OBJECT,
                  properties: {
                    employee_id: { type: Type.STRING, description: "The ID of the employee" },
                    behavioral_assessment: { type: Type.STRING, description: "1 sentence summary of their behavior" },
                    flight_risk: { type: Type.STRING, description: "'Low', 'Medium', or 'High'" },
                    recommended_action: { type: Type.STRING, description: "e.g., 'Promote', 'Retain', 'Action Plan'" },
                    reasoning: { type: Type.STRING, description: "Brief explanation of your decision" }
                  },
                  required: ["employee_id", "behavioral_assessment", "flight_risk", "recommended_action", "reasoning"]
                }
              }
            }
          });
        } catch (err: any) {
          if (attempt < 4 && (err?.message?.includes("503") || err?.status === 502 || err?.message?.includes("502") || err?.status === 429 || err?.message?.includes("429") || err?.message?.includes("quota") || err?.message?.includes("high demand") || err?.message?.includes("UNAVAILABLE"))) {
            console.log(`[Attempt ${attempt + 1}] Model ${modelToUse} unavailable or rate-limited, retrying in ${5000 * (attempt + 1)}ms...`);
      ...
      res.status(error?.status === 429 || error?.message?.includes('429') ? 429 : 500).json({ error: error?.message || "Failed to analyze HR data." });
    }
  });

  // API Route for testing Trends & Markov Analysis
  app.post("/api/analyze-trends", async (req, res) => {
    try {
          ...
          return await ai.models.generateContent({
            ...
            config: {
              ...
              responseSchema: {
                type: Type.OBJECT,
                properties: {
                  trendSummary: { type: Type.STRING, description: "3 sentence summary of trends" },
                  markovStates: {
                    type: Type.ARRAY,
                    items: {
                      type: Type.OBJECT,
                      properties: {
                        ...
                        probability: { type: Type.STRING, description: "e.g., '70%'" }
                      },
      ...
      res.status(error?.status === 429 || error?.message?.includes('429') ? 429 : 500).json({ error: error?.message || "Failed to analyze trends." });
    }
  });

  // API Route for Shift Generation
  app.post("/api/generate-shifts", async (req, res) => {
    try {
            ...
            config: {
              ...
              responseSchema: {
                ...
                items: {
                  ...
                  required: ["employee_id", "employee_name", "assigned_shifts", "reasoning"]
                }
              }
            }
          });
      ...
      res.status(error?.status === 429 || error?.message?.includes('429') ? 429 : 500).json({ error: error?.message || "Failed to generate shifts." });
    }
  });

  // API Route for Generator / Chatbot
  app.post("/api/chat", async (req, res) => {
    try {
      const { messages } = req.body;
      if (!Array.isArray(messages)) {
        return res.status(400).json({ error: "Invalid data format. Expected an array of messages." });
      }
      ...
      const doGenerate = async (attempt = 0): Promise<any> => {
        ...
        try {
          return await ai.models.generateContent({
            ...
            contents: prompt,
            config: {
              systemInstruction: "You are Pulse Assistant, a helpful AI integrated into an Enterprise Intelligence Platform. Provide concise, helpful answers regarding Human Resources, Marketing, Financials, and Supply Chain functionalities of this app.",
            }
          });
        } catch (err: any) {
          if (attempt < 4 && (err?.message?.includes("503") || err?.status === 502 || err?.message?.includes("502") || err?.status === 429 || err?.message?.includes("429") || err?.message?.includes("quota") || err?.message?.includes("high demand") || err?.message?.includes("UNAVAILABLE"))) {
      ...
      res.status(error?.status === 429 || error?.message?.includes('429') ? 429 : 500).json({ error: error?.message || "Failed to generate chat." });
    }
  });
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    // Check if dist folder exists, fallback if not
