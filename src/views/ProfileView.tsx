...
function renderMiniLogo(appId: string, bg: string, fallbackText: string) {
  switch (appId) {
    ...
    case "shopify":
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-emerald-500 shrink-0">
          <svg
            viewBox="0 0 100 100"
            className="w-5 h-5 text-emerald-950 fill-current"
          >
            <path d="M30 32h40l6 52H24l6-52z" />
            <path
              d="M40 32c0-8 4-14 10-14s10 6 10 14"
              fill="none"
              stroke="currentColor"
              strokeWidth="8"
              strokeLinecap="round"
    ...
    case "mailchimp":
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-yellow-400 shrink-0">
          <svg
            viewBox="0 0 100 100"
            className="w-5.5 h-5.5 text-yellow-950 fill-current"
          >
            <circle cx="50" cy="50" r="35" />
            <circle cx="40" cy="45" r="5" fill="#eab308" />
            <circle cx="60" cy="45" r="5" fill="#eab308" />
            <path
              d="M35 60c5 8 25 8 30 0"
              fill="none"
              stroke="#eab308"
              strokeWidth="6"
              strokeLinecap="round"
    ...
    case "gcp":
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-emerald-500 shrink-0 overflow-hidden">
          ...
          >
            <path
              d="M50 20C33.4 20 20 33.4 20 50s13.4 30 30 30c16.6 0 28-10.5 28-28H50v12h16c-1.5 8-8 12.5-16 12.5-10 0-18-8-18-18s8-18 18-18c5 0 9.5 2 13 5l9-9C66 25 59 20 50 20z"
              fill="#ffffff"
            />
          </svg>
        </div>
    ...
    case "aws":
      return (
        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-amber-600 shrink-0">
          ...
          >
            <path
              d="M20 55c15-18 45-18 60 0M80 50l3 7-8-1"
              strokeLinecap="round"
    ...
    default:
      return (
        <div
          className={`w-8 h-8 rounded-lg flex items-center justify-center font-black ${bg} text-white text-[11px] shrink-0 uppercase`}
        >
          {fallbackText}
        </div>
      );
...
export function ProfileView({
  ...
  useEffect(() => {
    ...
    const updateSimulatedCode = async () => {
      const secretToUse =
        generatedSecret || (tfaConfig?.enabled ? tfaConfig.secret : "");
      if (secretToUse) {
  ...
  const handleVerifyAndActivate = async (e: React.FormEvent) => {
    ...
    if (isValid) {
      ...
      setErrorMsg(
        "Verification code did not match. Please verify your system clock or copy code from simulator.",
      );
    }
  };
  ...
  const handleDisable2FA = () => {
    if (
      window.confirm(
        "WARNING: Disabling Two-Factor Authentication relaxes enterprise isolation security. Are you sure you want to proceed?",
      )
    ) {
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 max-w-4xl mx-auto pb-10 mt-6 select-none">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-sm text-left space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xs font-extrabold uppercase tracking-widest text-slate-700">
                Enterprise Profile & Sidebar divisions
              </h2>
              <p className="text-[10px] text-slate-400 font-medium leading-none mt-1">
                Configure company identifiers, custom menus, and synchronization
                keys.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:justify-end">
            ...
            <select
              value={preset}
              onChange={(e) => onSetPreset(e.target.value as PresetName)}
              className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-700 bg-white focus:outline-none focus:border-emerald-500"
              aria-label="Select website theme from profile"
        ...
        <form
          onSubmit={handleSaveTextChanges}
        ...
        >
          ...
          <div className="md:col-span-2 flex items-center justify-between pt-2">
            {errMessage && (
              <span className="text-rose-600 text-[10.5px] font-semibold">
                {errMessage}
              </span>
            )}
            {isSaved && (
              <span className="text-emerald-600 text-[10.5px] font-bold flex items-center gap-1">
                <Check className="w-3.5 h-3.5" /> Company profiles updated. Live
                menu optimized!
              </span>
            )}
        ...
        <div className="border-t pt-5 space-y-3">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium my-1">
              Select divisions you want present on your left-hand side corporate
              menus.
            </p>
          </div>
        ...
        <div className="border-t pt-5 space-y-3">
          <div>
            <h3 className="font-bold text-xs text-slate-800 uppercase tracking-wider">
              Configure division integrations
            </h3>
            <p className="text-[10px] text-slate-400 font-medium">
              Link custom software feeds corresponding to your enabled divisions
              above.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              ...PROFILE_APPS_MAP.filter((app) =>
                sidebarOptions.includes(app.cat),
              ),
            ].map((app) => {
              ...
              return (
                <div
                  ...
                  className={`p-3 border rounded-xl flex items-center justify-between text-xs transition-colors ${
                    isConnected
                      ? "border-blue-400 bg-emerald-50/5"
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <span className="font-bold text-slate-850 block leading-tight text-[11px]">
                        {app.name}
                      </span>
        ...
        <div className="flex items-center justify-between border-b pb-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-[10px] text-slate-400 font-medium leading-none mt-1">
                Multi-factor security layers for banking, payroll and financial
                tables.
              </p>
            </div>
          </div>
                ...
                <div className="md:col-span-8 space-y-2">
                  <div className="flex items-center gap-2 text-emerald-700 font-bold">
                    ...
                    <span className="text-sm">
                      Active MFA via{" "}
                      {tfaConfig.provider === "google"
                        ? "Google Authenticator"
                        : "Microsoft Authenticator"}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 leading-relaxed font-semibold">
                    Security status optimal. Code requests triggered on login
                    from unrecognized terminals, every 7 days, or when
                    visualizing ledger balance operations inside Financials.
                  </p>
                  <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mt-2">
                    MFA Provisioned Time:{" "}
                ...
                <div className="md:col-span-8 space-y-2">
                  <div className="flex items-center gap-2 text-rose-600 font-bold">
                    ...
                    <span className="text-sm font-extrabold uppercase tracking-wide">
                      Multi-Factor Unprovisioned
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 leading-relaxed font-medium">
                    Securing your platform with an authenticator app blocks raw
                    brute force credentials thefts. Linking your workspace with{" "}
                    ...
                    provides an immutable cryptographic fence.
                  </p>
                </div>
                <div className="md:col-span-4 flex justify-end">
                  <button
                  ...
                  >
                    <Key className="w-4 h-4 text-emerald-400" /> Set Up
                    Authenticator
                  </button>
                </div>
              </div>
          ...
          <div className="text-left space-y-6 animate-in fade-in slide-in-from-bottom-2">
            <div className="space-y-1">
              <h3 className="text-sm font-bold text-slate-800">
                Select Authenticator Ecosystem
              </h3>
              <p className="text-xs text-slate-500 font-medium">
                Choose whichever authenticator service you run inside your
                mobile smartphone or desktop.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-xs text-slate-500 mt-1 font-semibold leading-relaxed">
                  Support offline secure 2FA generation with standard 30-sec
                  algorithmic time-windows on Google Cloud.
                </p>
              ...
              >
                ...
                <p className="text-xs text-slate-500 mt-1 font-semibold leading-relaxed">
                  Excellent enterprise credentials syncing, support notification
                  alerts and passwordless validation.
                </p>
        ...
        {(setupStep === "scan" || setupStep === "verify") && (
          <div className="text-left animate-in fade-in slide-in-from-bottom-3 space-y-6">
                ...
                <div className="sm:col-span-7 space-y-4">
                  <div className="space-y-1">
                    <h3 className="font-bold text-sm text-slate-800">
                      Scan QR Code or Enter Secret Key Manual
                    </h3>
                    <p className="text-xs text-slate-500 leading-normal font-medium">
                      Open your camera within{" "}
                      {selectedProvider === "google" ? "Google" : "Microsoft"}{" "}
                      Authenticator app, scan this QR code, or paste the manual
                      key listed below:
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-150 p-3 rounded-xl flex items-center justify-between shadow-2xs select-text">
                    <div>
                      ...
                      <span className="text-xs font-black tracking-widest text-slate-700 select-all font-mono">
                        {generatedSecret}
                      </span>
                    </div>
                    <button
                  ...
                  <div className="p-3.5 bg-emerald-50 border border-emerald-100 rounded-xl leading-normal text-[10.5px] text-indigo-900 font-semibold flex items-start gap-2 select-text">
                    ...
                    <span>
                      Adding this profile creates an OTP link synchronized with
                      server epoch clock frequencies.
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4 text-left">
                  ...
                  <div className="py-2.5 flex items-center justify-between bg-slate-800/80 border border-white/10 px-4 rounded-xl">
                    <span className="text-2xl font-black tracking-widest text-[#5BC0BE] select-all font-mono">
                      {simulatedCode.substring(0, 3)}{" "}
                      {simulatedCode.substring(3, 6)}
                    </span>
                  ...
                  <p className="text-[10px] text-slate-450 leading-relaxed font-semibold italic text-slate-400">
                    *Tip: Copy these 6 digits directly to verify, or use your
                    real phone scanner. The codes are perfectly identical!
                  </p>
                </div>
              </div>
            </div>
            ...
            <form
              ...
              className="pt-6 border-t border-slate-100 max-w-lg space-y-4"
            >
              <div className="space-y-1.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 border border-slate-100 rounded-lg hover:border-slate-200 transition-colors text-left">
              ...
              <p className="text-sm text-slate-500 mt-1">
                English (United States), Timezone: Pacific/Los Angeles.
              </p>
            </div>
            <div className="p-4 border border-slate-100 rounded-lg hover:border-slate-200 transition-colors text-left">
              <div className="font-semibold text-slate-800">
              ...
              <p className="text-sm text-slate-500 mt-1">
                Email recaps and push notifications enabled.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 text-left">
          <h2 className="text-sm font-bold uppercase tracking-widest text-slate-700 mb-6 flex items-center gap-2">
            <Activity className="w-4 h-4 text-emerald-500" /> Security Status &
            Logs
          </h2>
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              <div className="text-left">
                <div className="text-sm font-semibold text-slate-800">
                  Granted standard enterprise access scope
                </div>
