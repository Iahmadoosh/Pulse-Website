...
const PLATFORM_DETAILS = {
  instagram: { name: 'Instagram Broadcasts', desc: 'Engage local visual pottery and ceramic enthusiasts with high resonance media briefs.', placeholderHandle: 'e.g. @artisan_boutique', buttonColor: 'bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90', helperText: 'Requires Instagram Graph Client Access Token scopes.' },
  facebook: { name: 'Facebook Pages Directory', desc: 'Saturate neighborhood event groups and community ceramic craft lists.', placeholderHandle: 'e.g. fb.me/artisan_pottery', buttonColor: 'bg-[#1877F2] hover:bg-[#1565C0]', helperText: 'Syncs via Facebook Graph Client Secret scopes.' },
  tiktok: { name: 'TikTok Shop Stream', desc: 'Distribute quick interactive sound-bite processes from the wheel kiln directly to design trends.', placeholderHandle: 'e.g. @clay_process', buttonColor: 'bg-[#000000] hover:bg-slate-900 border border-slate-800', helperText: 'Requires TikTok Developer Client authorization tokens.' },
  x: { name: 'X Social Platform Network', desc: 'Engage micro-blog announcements, product drops, and trade threads.', placeholderHandle: 'e.g. @ArtisanCeramics', buttonColor: 'bg-[#0F1419] hover:bg-[#000000]', helperText: 'Linked via Twitter Developer OAuth v2 standard API keys.' },
  reddit: { name: 'Reddit Ceramics Subs', desc: 'Publish storytelling posts to community craft focus threads.', placeholderHandle: 'e.g. u/clay_craftsman', buttonColor: 'bg-[#FF4500] hover:bg-[#E03D00]', helperText: 'Linked via Reddit Client API secret keys.' },
  google_calendar: { name: 'Google Calendar API', desc: 'Export scheduled social briefs and campaign slots directly into your real-time Google Calendar.', placeholderHandle: 'e.g. marketing-team@gmail.com', buttonColor: 'bg-[#4285F4] hover:bg-[#3273DC]', helperText: 'Securely syncs marketing campaigns to corporate Google Calendar feeds.' },
};
...
const renderPlatformLogo = (platform: string) => {
  switch (platform) {
    case 'instagram':
      return (
        <svg className="w-5 h-5 text-pink-600 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    case 'tiktok':
      return (
        <svg className="w-5 h-5 text-slate-900 shrink-0" viewBox="0 0 24 24" fill="currentColor">
...
const MOCK_TRENDS = [
  { id: 't-1', topic: '#SmallBusinessSaturday', volume: '14,200', change: '+34%', sentiment: 'Positive' },
  { id: 't-2', topic: 'Wabi-Sabi Ceramics Design', volume: '8,400', change: '+92%', sentiment: 'Positive' },
  { id: 't-3', topic: 'Bespoke Dark Indigo Mugs', volume: '5,900', change: '+41%', sentiment: 'Positive' },
  { id: 't-4', topic: 'Artisanal Clay Kiln Methods', volume: '4,100', change: '+18%', sentiment: 'Neutral' },
];
...
const INITIAL_CAMPAIGNS: Campaign[] = [
  { id: 1, name: 'Local Outreach (Summer)', status: 'Active', budget: 500, spend: 120, conversions: 34 },
  { id: 2, name: 'New Product Teaser', status: 'Paused', budget: 150, spend: 150, conversions: 12 },
  { id: 3, name: 'Retargeting Flow A', status: 'Active', budget: 100, spend: 42, conversions: 8 },
];
...
Provide advice on improving Cost-per-Conversion. Format your answers in professional, highly concise conversational Markdown suitable for minor retailers. Keep recommendations under 3 bullets.`;
  ...
  return (
    <div id="campaigns-tab-root" className="space-y-8 animate-in fade-in duration-300">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-6 pb-2 border-b border-slate-100">
              <h4 className="text-[11px] font-black uppercase tracking-widest text-[#1E293B]">Campaign Management</h4>
              ...
              >
                <Plus className="w-3.5 h-3.5 text-emerald-600" /> Create Campaign Target
              </button>
            </div>
              ...
              <div className="min-w-[600px] space-y-1">
                ...
                {campaigns.length === 0 ? (
                  <div className="p-8 text-center text-xs text-slate-400">
                    No active campaign targets registered. Sprout one above to begin auditing conversions.
                  </div>
                ) : (
                  campaigns.map(c => {
                    const costPerC = c.conversions > 0 ? (c.spend / c.conversions).toFixed(2) : '0.00';
                    return (
                      <div key={c.id} className="grid grid-cols-12 gap-4 items-center py-4 border-b border-slate-100 hover:bg-slate-50/50 rounded-xl px-3 transition-colors">
                        
                        {/* Col 1: Campaign Identifier */}
                        <div className="col-span-4 block min-w-0 truncate">
                          <h5 className="font-extrabold text-slate-800 text-xs leading-snug truncate" title={c.name}>{c.name}</h5>
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          >
                            <span className={`w-1.5 h-1.5 rounded-full ${c.status === 'Active' ? 'bg-[#10B981]' : 'bg-slate-400'}`} />
                        ...
                        <div className="col-span-3">
                          <div className="flex flex-col text-left">
                            <span className="text-xs font-extrabold text-slate-800 font-sans">${c.budget}</span>
                            <span className="text-[10px] text-slate-400 mt-0.5">Spent sum: ${c.spend}</span>
                          </div>
                        </div>
                        ...
                        <div className="col-span-2">
                          <div className="flex flex-col text-left">
                            <span className="text-xs font-extrabold text-[#10B981] font-sans">{c.conversions}</span>
                            <span className="text-[10px] text-slate-400 mt-0.5 font-medium">${costPerC}/conv</span>
                          </div>
                        </div>
        ...
        <div className="bg-[#E6F7F0]/45 border border-emerald-100 rounded-2xl p-6 shadow-xs flex flex-col justify-between h-full min-h-[340px]">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-[11px] font-black uppercase tracking-widest text-emerald-800">AI Campaign Intelligence</h4>
            </div>

            <p className="text-xs text-[#065F46] leading-relaxed font-semibold">
              Your "Local Outreach" channel is executing at 2x efficiency. Consider requesting a complete AI Budget analysis to shift unused paused campaign balances.
            </p>
          ...
          <div className="mt-5 pt-4">
            ...
            >
              ...
              {isOptimizing ? 'Analyzing & Optimizing...' : 'Analyze & Optimize Budgets'}
            </button>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-6 shadow-xl animate-in zoom-in-95 duration-200 text-left">
            <div className="flex items-center justify-between mb-4 border-b pb-3 border-slate-100">
              <h3 className="font-extrabold text-xs uppercase tracking-widest text-[#1E293B] flex items-center gap-2">
                <Megaphone className="w-4 h-4 text-emerald-500" /> Spawn New Campaign
              </h3>
              <button 
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-4">
              <div className="space-y-1.5 text-left">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Campaign Identifier Name</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5 text-left">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Allocated Budget ($)</label>
                  <input
                    type="number"
                    required
                    min="1"
                ...
                <div className="space-y-1.5 text-left">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Initial Status</label>
                  <select
                    value={newStatus}
...
function SocialTrendsTab() {
  ...
  const handleGenerateIdeas = async () => {
    ...
    const trendsStr = MOCK_TRENDS
      .map(t => `- ${t.topic}: Volume ${t.volume}, Sentiment ${t.sentiment}, Change ${t.change}`)
      .join('\n');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row gap-6">
        <div className="md:w-2/3 bg-white border border-slate-200 rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
             <h3 className="text-sm font-bold uppercase tracking-widest text-slate-700 flex items-center gap-2">
               <Hash className="w-4 h-4 text-emerald-500" /> Local & Industry Trends
             </h3>
             <span className="text-xs font-bold text-slate-450 uppercase">{new Date().toLocaleDateString()}</span>
          </div>
          
          <div className="space-y-4">
            {MOCK_TRENDS.map(trend => (
              <div key={trend.id} className="flex items-center justify-between p-4 border border-slate-100 rounded-lg hover:border-slate-200 transition-colors">
                <div className="flex flex-col">
                  <span className="font-bold text-slate-800 text-lg">{trend.topic}</span>
                  <span className="text-xs font-medium text-slate-500 mt-1 flex items-center gap-1">
                    Volume: <span className="text-slate-700">{trend.volume} mentions</span>
                  </span>
                </div>
                <div className="flex items-center gap-6">
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">Sentiment</span>
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                      trend.sentiment === 'Positive' ? 'bg-emerald-100 text-emerald-700' : 
                      trend.sentiment === 'Negative' ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'
                     }`}>
                      {trend.sentiment}
                    </span>
                  </div>
                  <div className="flex flex-col items-end w-16">
                     <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">Impact</span>
                     <span className="text-sm font-bold text-emerald-600 flex items-center gap-0.5">
                       {trend.change} <ArrowUpRight className="w-3 h-3 text-emerald-500" />
                     </span>
                  </div>
                </div>
              </div>
        ...
        <div className="md:w-1/3 flex flex-col gap-6">
           <div className="bg-emerald-600 rounded-xl p-6 shadow-sm text-white flex flex-col justify-between">
             <div className="flex items-center gap-2 mb-2 opacity-80">
               ...
               <h3 className="text-xs font-bold uppercase tracking-widest">AI Trend Analysis</h3>
             </div>
             <p className="text-lg font-medium leading-tight mb-4">
               "Wabi-Sabi Ceramics Design" is trending early this month. Prepare teasers, email briefs, and custom campaign flows immediately to cash in.
             </p>
             <button 
               onClick={handleGenerateIdeas}
               className="bg-white text-emerald-600 font-bold text-xs uppercase tracking-widest py-2.5 px-4 rounded-xl hover:bg-slate-50 transition-colors self-start cursor-pointer shadow-sm"
             >
               Generate Campaign Ideas
             </button>
           </div>
           
           <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 flex flex-col items-center justify-center text-center h-full min-h-[200px]">
             <Activity className="w-8 h-8 text-slate-300 mb-3" />
             <p className="text-sm text-slate-505 font-medium max-w-[200px]">Interactive local audience engagement forecasts peak between 2PM and 4PM today.</p>
           </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl p-6 shadow-xl animate-in zoom-in-95 duration-200 text-left">
            <div className="flex items-center justify-between mb-4 border-b pb-3">
              <h3 className="font-extrabold text-xs uppercase tracking-widest text-slate-700 flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-emerald-500" /> AI Campaign Ideator
              </h3>
              <button 
            ...
            {isGenerating ? (
              <div className="py-16 flex flex-col items-center justify-center text-emerald-750 space-y-4">
                ...
                <div className="text-center space-y-1">
                  <p className="text-xs font-bold animate-pulse uppercase tracking-wider">Analyzing volume matrices...</p>
                  <p className="text-[10px] text-slate-400">Synthesizing targeted marketing recipes...</p>
                </div>
              </div>
            ) : campaignIdeas ? (
              <div className="space-y-4 font-sans">
                <div className="text-xs text-slate-700 leading-relaxed max-h-96 overflow-y-auto bg-slate-50 border border-slate-100 p-5 rounded-2xl shadow-inner select-text prose prose-sm prose-indigo">
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    onClick={handleCopy}
                    className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider py-3 rounded-xl transition-all shadow-sm flex items-center justify-center gap-2 cursor-pointer"
                  >
                    {copied ? 'Copied with Success!' : 'Copy Playbook'}
                  </button>
                  <button
                    onClick={handleGenerateIdeas}
                    className="flex-1 border text-slate-750 border-slate-250 text-slate-700 font-bold text-xs uppercase tracking-wider py-3 rounded-xl transition-colors hover:bg-slate-50 cursor-pointer"
                  >
                    Regenerate Playbook
                  </button>
                </div>
              ...
              <p className="text-xs text-slate-500">Failed to boot content synthesis flow.</p>
            )}
          </div>
        </div>
      )}
...
function SocialAccountsTab({ connectedAccounts, setConnectedAccounts }: SocialAccountsTabProps) {
  const [connectingPlatform, setConnectingPlatform] = useState<ConnectedAccount['platform'] | null>(null);
  ...
  const platforms: ConnectedAccount['platform'][] = ['instagram', 'facebook', 'tiktok', 'x', 'reddit', 'google_calendar'];
      ...
      <div className="bg-emerald-50/60 border border-emerald-100 rounded-xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h3 className="font-bold text-emerald-950 flex items-center gap-1.5 text-sm uppercase tracking-wider">
            <ShieldCheck className="w-5 h-5 text-emerald-600" /> Authorized Multi-Channel OAuth Connections
          </h3>
          <p className="text-xs text-emerald-850 text-emerald-800 leading-relaxed max-w-2xl mt-1">
            Authorize linked social media handles securely. Once completed, load user triggers within the Content Studio tab/Automation workflows to drive dispatches into physical streams.
          </p>
        </div>
          ...
          return (
            <div 
              ...
              className={`bg-white border rounded-xl overflow-hidden p-6 shadow-sm flex flex-col justify-between hover:border-slate-300 hover:shadow-md transition-all h-full ${
                matched ? 'border-emerald-100 bg-emerald-50/10' : 'border-slate-200'
              }`}
            >
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    <span className={`w-3 h-3 rounded-full ${matched ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                    {renderPlatformLogo(plat)}
                    <h4 className="font-bold text-slate-800 text-sm">{meta?.name || plat}</h4>
                  </div>
                  {matched ? (
                ...
                {matched && (
                  <div className="mt-4 p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1 font-sans text-[11px] leading-relaxed">
                    <div className="flex justify-between">
                      <span className="text-slate-400 font-medium font-sans">Synced handle:</span>
                      <span className="font-bold text-emerald-955 text-slate-800">{matched.handle}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400 font-medium">Synced timestamp:</span>
                      <span className="text-slate-600 font-sans">{matched.connectedAt}</span>
                    </div>
                  </div>
                )}
              </div>
                ...
                {isSelected ? (
                  <div className="space-y-4 bg-slate-50 p-4 border rounded-xl animate-in slide-in-from-top duration-300">
                    <div className="flex items-center justify-between border-b pb-2">
                      <span className="text-[10px] font-bold text-slate-800 uppercase font-sans">Configure metadata credentials</span>
                      <button 
                    ...
                    <div className="space-y-3 font-sans">
                      <div>
                        <label className="block text-[9px] font-black uppercase text-slate-500 tracking-wider mb-1">Account Handle / Brand Identifier</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-[9px] font-black uppercase text-slate-500 tracking-wider mb-1">Key App Token Identifier ID</label>
                        <input
                          type="text"
                      ...
                      >
                        ...
                        {isVerifying ? 'Synchronizing scope parameters...' : 'Verify secure OAuth Handshake'}
                      </button>
                    </div>
                  </div>
                ) : matched ? (
...
export function MarketingView() {
    ...
    try {
      ...
      if (stored) {
        ...
        const initialDefault: ConnectedAccount[] = [
          { platform: 'instagram', handle: '@artisan_ceramics', appId: 'META-INST-902', token: 'IG_EAAF8B...', connectedAt: '2026-05-12' },
          { platform: 'x', handle: '@ArtisanCeramics', appId: 'X-DEV-8721', token: 'TW_BH64HG...', connectedAt: '2026-05-20' }
        ];
        setConnectedAccounts(initialDefault);
        localStorage.setItem('pulse_social_accounts', JSON.stringify(initialDefault));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div id="marketing-view-grand-layout" className="flex flex-col h-full animate-in fade-in duration-500 max-w-6xl mx-auto pb-10 mt-6 select-none leading-normal">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-500 mt-1">
            Authorise target handles, generate A/B copywriting variations, map demographic buyer vectors, and run multi-node visual automation pipes.
          </p>
        </div>
